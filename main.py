...
# bird image
bird = pygame.image.load(fr'images/bird1.png')

#Game over letters images 
g = pygame.image.load(r'images\G.png')
a = pygame.image.load(r'images\A.png')
m = pygame.image.load(r'images\M.png')
e = pygame.image.load(r'images\E.png')
o = pygame.image.load(r'images\O.png')
v = pygame.image.load(r'images\V.png')
r= pygame.image.load(r'images\R.png')

#press to start image
press = pygame.image.load(r'images\press.png') 

# sounds variabels
success = pygame.mixer.Sound(r'sounds\success.wav')
fail = pygame.mixer.Sound(r'sounds\fall.wav')
game_over_sound = pygame.mixer.Sound(r'sounds\game_over.mp3')
...
def Game_Over():
    ...
    if not game_over and flying == True:
        ...
        if not tries:
            # this is to make a motion for 'Game over' word
            global speed_counter
            global index
            # start to play game over voice at index 2
            if index == 2:
                  game_over_sound.play()
            speed_counter += 1
            if speed_counter > letter_speed: # this controls the speed of motion
                if index > 7:
                    pass
                else:
                    game_over_group.add(letters_list[index])
                    speed_counter = 0
                    index += 1
            # this to draw buttons after the motion of 'game over' ends        
            if index > 7:
...
def reset_game():
    ...
    flappy.rect.y = screen_height//2
    score = 0
    #removing the objects of the letters and reset the index of ' letters_list'
    G.kill(), A.kill(), M.kill(), E1.kill(), O.kill(), V.kill(), E2.kill(), R.kill()
    global index
    index = 0
...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        pygame.sprite.Sprite.__init__(
            self)  # inherit some functionality (from pygame.sprite.Sprite) class
        self.images = []
        self.index = 0
        self.counter = 0
        ...
        for num in range(1, 4):
            img = pygame.image.load(fr'images/bird{num}.png')
...
# This is a class to show letters of the word of 'Game Over'
class Letter(pygame.sprite.Sprite):
    def __init__(self, img_letter, x, y):
        pygame.sprite.Sprite.__init__(self)
        self.image = img_letter
        self.rect = self.image.get_rect()
        self.rect.center = [x, y]
...
reset_button = Button(screen_width//2 - 150 ,
                      screen_height//2 - 50, restart_button)
# create quit button instance
quit_button = Button(screen_width//2 + 50, screen_height//2 - 50, exit_button)

# instantiate objects from letter class
G = Letter(g, 130, screen_height // 2 - 200)
A = Letter(a, 213, screen_height // 2 - 200)
M = Letter(m, 296, screen_height // 2 - 200)
E1 = Letter(e, 379, screen_height // 2 - 200)
O = Letter(o, 472, screen_height // 2 - 200)
V = Letter(v, 555, screen_height // 2 - 200)
E2 = Letter(e, 638, screen_height // 2 - 200)
R = Letter(r, 721, screen_height // 2 - 200)

letters_list = [G, A, M, E1, O, V, E2, R]  #list of objects of letters word 'Game Over'
index = 0  # index of previous list
game_over_group = pygame.sprite.Group()  # a group for the sprites of the 'Game Over' letters
...
while run:
    ...
    #show the 'press to start' image at the beginning of every game
    if flying == False and tries == 5:
        screen.blit(press, (screen_width // 2 - 250, screen_height // 2 - 25))
    ...
    best = read_score()
    draw_text(f'High Score: {best}', font, color, screen_width-300, 20)
